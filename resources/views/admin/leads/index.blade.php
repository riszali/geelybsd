...
<body class="min-h-screen pb-16 selection:bg-[#00b4d8] selection:text-black antialiased">
    ...
    <div id="toastNotification" class="fixed top-5 right-5 z-50 transform translate-y-[-150%] opacity-0 transition-all duration-300 pointer-events-none">
        <div class="hud-card px-4 py-3 flex items-center gap-3 border border-cyan-500/40 shadow-[0_0_25px_rgba(0,229,255,0.2)] bg-[#0d131a]/95 backdrop-blur-md">
        ...
        <div class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-2 rounded-xl bg-red-500/10 border border-red-500/30 hover:bg-red-500 text-red-400 hover:text-white text-xs font-semibold uppercase tracking-wider transition-all">
    ...
    <main class="w-full max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 pt-5 sm:pt-7 space-y-5 sm:space-y-7">

        <!-- ======================================================================
             BAGIAN 1: STATISTIK KUNJUNGAN WEB (100% DATA REAL)
        ====================================================================== -->
        <section class="space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="text-[10px] sm:text-[11px] text-gray-400 tracking-wider">Statistik Pengunjung Realtime</span>
            </div>

            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
                <div class="hud-card p-3.5 sm:p-5 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-24 h-24 bg-cyan-500/10 rounded-full blur-[25px] pointer-events-none"></div>
                    <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 block mb-1">Total Pageviews</span>
                    <div class="font-geely text-2xl sm:text-4xl text-white">{{ number_format($analytics['total_views'] ?? 0) }}</div>
                    <span class="text-[10px] sm:text-[11px] text-[#00b4d8] mt-1.5 block font-medium">+{{ number_format($analytics['today_views'] ?? 0) }} hari ini</span>
                </div>

                <div class="hud-card p-3.5 sm:p-5 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-24 h-24 bg-indigo-500/10 rounded-full blur-[25px] pointer-events-none"></div>
                    <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 block mb-1">Pengunjung Unik</span>
                    <div class="font-geely text-2xl sm:text-4xl text-white">{{ number_format($analytics['total_uniques'] ?? 0) }}</div>
                    <span class="text-[10px] sm:text-[11px] text-indigo-400 mt-1.5 block font-medium">+{{ number_format($analytics['today_uniques'] ?? 0) }} hari ini</span>
                </div>

                <div class="hud-card p-3.5 sm:p-5 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-24 h-24 bg-emerald-500/10 rounded-full blur-[25px] pointer-events-none"></div>
                    <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 block mb-1">Perangkat (Device)</span>
                    ...
                    <div class="w-full bg-white/10 h-1.5 rounded-full overflow-hidden mt-2 sm:mt-2.5 flex">
                ...
                <div class="hud-card p-3.5 sm:p-5 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-24 h-24 bg-pink-500/10 rounded-full blur-[25px] pointer-events-none"></div>
                    <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 block mb-1">Conversion Rate</span>
                    @php
                    ...
                    <span class="text-[10px] sm:text-[11px] text-gray-400 mt-1.5 block font-medium truncate">{{ $totalLeadsCount }} leads / {{ $analytics['total_uniques'] ?? 0 }} visitor</span>
                </div>
            </div>

            <div class="hud-card p-4 sm:p-6 relative overflow-hidden">
                ...
                <!-- 3 Kotak Metrik Ringkasan (Real dari Database) -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2.5 sm:gap-4 mt-5 pt-5 border-t border-white/10">
                    <div class="hud-card-sm p-3.5 sm:p-4 text-center flex flex-col items-center justify-center">
                        <span class="text-[10px] sm:text-[11px] font-bold tracking-[0.16em] uppercase text-gray-400 block mb-1.5">
                            RATA-RATA PENGUNJUNG/HARI
                        </span>
                        <div class="font-orbitron text-2xl sm:text-3xl text-[#00E5FF] font-bold tracking-tight">
                            {{ $analytics['avg_visitors_per_day'] ?? 0 }}
                        </div>
                    </div>
                    ...
                    <div class="hud-card-sm p-3.5 sm:p-4 text-center flex flex-col items-center justify-center">
                        ...
                        <div class="font-orbitron text-2xl sm:text-3xl text-[#D946EF] font-bold tracking-tight">
                            {{ $analytics['ratio_visitor_to_lead'] ?? 0 }}%
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="hud-card p-4 sm:p-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
                    @forelse($analytics['top_pages'] ?? [] as $index => $page)
                        <div class="flex items-center justify-between text-xs p-2.5 sm:p-3 rounded-xl bg-white/5 border border-white/5">
        ...
        <!-- ======================================================================
             BAGIAN 2: PIPELINE LEADS CRM (REAL-TIME DATABASE)
        ====================================================================== -->
        <section id="pipeline" class="space-y-4 pt-2">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div class="text-xs text-gray-400 tracking-wider">
                    Total Masuk: <span class="text-white font-bold">{{ $stats['total'] ?? 0 }}</span> leads
                </div>
            </div>

            <div class="flex overflow-x-auto custom-scrollbar gap-2 sm:gap-2.5 pb-2 -mx-3 px-3 sm:mx-0 sm:px-0">
                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-[#00b4d8] hover:bg-white/5 transition-all {{ !request('status') ? 'ring-1 ring-[#00b4d8]/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-gray-400 tracking-wider block">Semua Lead</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['total'] ?? 0 }}</div>
                </a>

                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['status' => 'new', '#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-cyan-400 hover:bg-white/5 transition-all {{ request('status') === 'new' ? 'ring-1 ring-cyan-400/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-cyan-400 tracking-wider block">Baru (New)</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['new'] ?? 0 }}</div>
                </a>

                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['status' => 'contacted', '#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-amber-400 hover:bg-white/5 transition-all {{ request('status') === 'contacted' ? 'ring-1 ring-amber-400/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-amber-400 tracking-wider block">Dihubungi</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['contacted'] ?? 0 }}</div>
                </a>

                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['status' => 'scheduled', '#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-blue-400 hover:bg-white/5 transition-all {{ request('status') === 'scheduled' ? 'ring-1 ring-blue-400/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-blue-400 tracking-wider block">Scheduled</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['scheduled'] ?? 0 }}</div>
                </a>

                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['status' => 'deal', '#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-emerald-400 hover:bg-white/5 transition-all {{ request('status') === 'deal' ? 'ring-1 ring-emerald-400/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-emerald-400 tracking-wider block">SPK / Deal</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['deal'] ?? 0 }}</div>
                </a>

                <a href="{{ route('admin.leads.index', array_merge(request()->except('status'), ['status' => 'lost', '#pipeline'])) }}" 
                   class="hud-card-sm p-3 sm:p-3.5 min-w-[125px] sm:min-w-[140px] flex-1 block border-l-4 border-rose-400 hover:bg-white/5 transition-all {{ request('status') === 'lost' ? 'ring-1 ring-rose-400/40 bg-white/[0.04]' : '' }}">
                    <span class="text-[9px] sm:text-[10px] font-bold uppercase text-rose-400 tracking-wider block">Drop / Lost</span>
                    <div class="font-geely text-xl sm:text-2xl text-white mt-1">{{ $stats['lost'] ?? 0 }}</div>
                </a>
            </div>

            <div class="hud-card p-3 sm:p-3.5 flex flex-wrap items-center justify-between gap-2.5">
                <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    ...
                    <a href="{{ route('admin.leads.index', array_merge(request()->except('type'), ['#pipeline'])) }}" 
                       class="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all {{ !request('type') ? 'bg-[#00b4d8] text-black shadow-[0_0_15px_rgba(0,180,216,0.4)]' : 'bg-white/5 text-gray-300 hover:bg-white/10' }}">
                        Semua Channel
                    </a>

                    <a href="{{ route('admin.leads.index', array_merge(request()->except('type'), ['type' => 'test_drive', '#pipeline'])) }}" 
                       class="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all flex items-center gap-1.5 {{ request('type') === 'test_drive' ? 'bg-cyan-500 text-black shadow-[0_0_15px_rgba(6,182,212,0.4)]' : 'bg-white/5 text-cyan-300 hover:bg-white/10' }}">
                    ...
                    <a href="{{ route('admin.leads.index', array_merge(request()->except('type'), ['type' => 'whatsapp_inquiry', '#pipeline'])) }}" 
                       class="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all flex items-center gap-1.5 {{ request('type') === 'whatsapp_inquiry' ? 'bg-emerald-500 text-black shadow-[0_0_15px_rgba(16,185,129,0.4)]' : 'bg-white/5 text-emerald-300 hover:bg-white/10' }}">
                    ...
                    <a href="{{ route('admin.leads.index', array_merge(request()->except('type'), ['type' => 'credit_simulation', '#pipeline'])) }}" 
                       class="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all flex items-center gap-1.5 {{ request('type') === 'credit_simulation' ? 'bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.4)]' : 'bg-white/5 text-indigo-300 hover:bg-white/10' }}">
            ...
            <div class="hud-card p-3.5 sm:p-4">
                <form action="{{ route('admin.leads.index') }}#pipeline" method="GET" class="flex flex-col md:flex-row gap-2.5 sm:gap-3">
                    ...
                    <div class="relative flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" 
                               ...
                               class="w-full bg-[#0a0f16] border border-white/10 rounded-xl px-4 py-2.5 pl-10 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#00b4d8]">
                    ...
                    <select name="model" class="bg-[#0a0f16] border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-[#00b4d8]">
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 md:flex-none px-5 py-2.5 rounded-xl bg-[#00b4d8] text-black font-bold text-xs uppercase tracking-wider hover:bg-[#00c4e8] transition-all">
                            Filter
                        </button>
                        @if(request()->hasAny(['search', 'model', 'status', 'type']))
                            <a href="{{ route('admin.leads.index') }}#pipeline" class="px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-300 hover:text-white text-xs font-semibold flex items-center justify-center">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="border-b border-white/10 bg-[#0a0f16] text-[10px] uppercase font-bold tracking-widest text-gray-400">
                                <th class="py-3.5 px-5">Pelanggan & Kontak</th>
                                <th class="py-3.5 px-5">Channel & Model</th>
                                <th class="py-3.5 px-5">Detail Permintaan</th>
                                <th class="py-3.5 px-5">Status Pipeline</th>
                                ...
                                <tr class="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="py-3.5 px-5">
                                        ...
                                        @elseif(($lead->type ?? '') === 'whatsapp_inquiry')
                                            <div class="text-gray-300 truncate max-w-[280px]" title="{{ $lead->source }}">
            ...
            <div class="space-y-2.5 block md:hidden">
                @forelse($leads as $lead)
                    <div class="hud-card p-3.5 space-y-2.5">
                ...
                @empty
                    <div class="hud-card p-6 text-center text-xs text-gray-500">
                        Belum ada data prospek yang sesuai kriteria pencarian.
                    </div>
                @endforelse
    ...
    <!-- Raw Data dari Database 100% Real (Tanpa Mock Data Dummy) -->
    <script id="chartRawData" type="application/json">
        {!! json_encode($analytics['chart_data'] ?? []) !!}
    </script>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Inisialisasi Chart.js Dual Axis 100% Real dari Database
            ...
            if (canvas) {
                ...
                let rawChartData = [];
                try {
                    const rawDataEl = document.getElementById('chartRawData');
                    if (rawDataEl && rawDataEl.textContent.trim()) {
                        rawChartData = JSON.parse(rawDataEl.textContent.trim());
                    }
                } catch (e) {
                    rawChartData = [];
                }

                const labels = Array.isArray(rawChartData) ? rawChartData.map(d => d.label || '') : [];
                const visitorData = Array.isArray(rawChartData) ? rawChartData.map(d => Number(d.views) || 0) : [];
                const leadData = Array.isArray(rawChartData) ? rawChartData.map(d => Number(d.leads) || 0) : [];
                ...
                const maxVisitorVal = visitorData.length > 0 ? Math.max(...visitorData) : 0;
                const maxLeadVal = leadData.length > 0 ? Math.max(...leadData) : 0;
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            yVisitors: {
                                ...
                                position: 'left',
                                beginAtZero: true,
                                suggestedMax: maxVisitorVal > 0 ? Math.ceil(maxVisitorVal * 1.2) : 10,
                                grid: {
                                    color: 'rgba(255, 255, 255, 0.05)',
                                    drawBorder: false,
                                },
                            ...
                            yLeads: {
                                ...
                                position: 'right',
                                beginAtZero: true,
                                suggestedMax: maxLeadVal > 0 ? Math.ceil(maxLeadVal * 1.2) : 5,
                                grid: {
